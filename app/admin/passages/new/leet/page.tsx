...
export default function NewLEETPage() {
  const [year, setYear] = useState(2026);
  const [leetType, setLeetType] = useState(""); // 추리논증, 언어이해
  ...
  const parseText = (text: string) => {
    ...
    const sourcePatterns = [/202\d+학년도\s*LEET/, /LEET\s*언어이해/, /LEET\s*추리논증/];
