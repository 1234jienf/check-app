...
/**
 * 선생님만 호출. 본문은 작고, 실제 PDF는 클라이언트가 Supabase Storage에 직접 업로드 (Vercel 4.5MB 제한 회피).
 */
  ...
  try {
    ...
    const rawExt =
      typeof body.extension === "string"
        ...
        : "";
    const safeExt = ALLOWED_EXT.has(rawExt) ? rawExt : "";

    if (!safeExt) {
      return NextResponse.json({ error: "PDF만 업로드할 수 있습니다." }, { status: 400 });
    }
    ...
    const objectPath = `dasang-hwpx/${timestamp}-${randomStr}.${safeExt}`;
    ...
    console.error("dasang-pdf-presign:", e);
